...
import {
  mapZakkaStockRawRow,
  executeZakkaStockImportRow,
  determineCounterpartyType,
  type ZakkaStockRawRow,
  type MappedZakkaStockRow,
  type ExecuteResult,
} from "../../lib/api/zakkaStockImport";
...
const DEFAULT_START_ROW = 529;
const DEFAULT_END_ROW = 881;

// 列番号(1始まり、シート実物のヘッダー行で確認済み)
const COL = {
  purchaseDate: 3, // C 仕入日
  itemName: 4, // D 仕入品名
  sourceMain: 5, // E 仕入先
  sourceSub: 6, // F 仕入先2
  usedGoodsLabel: 7, // G 新品・古物判定
  purchasePrice: 8, // H 仕入高
};
...
/** 仕入品名・仕入日がすべて空欄の行は空行とみなして取込対象から除外する。 */
function isBlankRow(row: unknown[]): boolean {
  return toStr(cell(row, COL.itemName)) == null && cell(row, COL.purchaseDate) == null;
}

function buildRawRows(aoa: unknown[][], startRow: number, endRow: number): ZakkaStockRawRow[] {
  const result: ZakkaStockRawRow[] = [];
...
function markInBatchDuplicates(rows: MappedZakkaStockRow[]): void {
...
const OUTCOME_LABELS: Record<MappedZakkaStockRow["outcome"], string> = {
  new: "新規",
  duplicate: "重複(スキップ)",
  error: "エラー",
};

const OUTCOME_COLORS: Record<MappedZakkaStockRow["outcome"], string> = {
  new: "var(--text-primary, inherit)",
  duplicate: "var(--text-muted)",
  error: "var(--danger-text)",
};

export default function ZakkaStockImportPanel() {
  ...
  const [mappedRows, setMappedRows] = useState<MappedZakkaStockRow[] | null>(null);
  const [filter, setFilter] = useState<"all" | MappedZakkaStockRow["outcome"]>("all");
  ...
  async function handleDryRun() {
    ...
    try {
      ...
      // 取引先区分は「仕入先」列の文字列から自動判定する(2026-09-03ユーザー指示。
      // ブックオフ/駿河屋→インボイスあり、手持品/ラクマ/メルカリ/ヤフー→消費者(個人)、それ以外→インボイスあり)。
      // ドライラン結果の表で行ごとに個別上書きも可能。
      const mapped = rawRows.map((raw) => mapZakkaStockRawRow(raw, defaultStatus, determineCounterpartyType(raw.sourceMain)));
  ...
  async function handleExecute() {
    ...
    for (const row of targets) {
      const result = await executeZakkaStockImportRow(row);
  ...
  const FILTER_OPTIONS: { key: "all" | MappedZakkaStockRow["outcome"]; label: string; count: number | null }[] = [
  ...
  return (
    <div>
      <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 0, marginBottom: 16 }}>
        「仕入・販売帳」エクセルシートの雑貨在庫ブロック(既定は529〜881行、C〜H列: 仕入日・仕入品名・仕入先・
        仕入先2・新品古物判定・仕入高)を読み込み、商品・仕入データとして一括登録します(売上データは作成されません)。
        カテゴリは行内容に関わらず一律「雑貨」として取り込みます。管理番号列がこのブロックには無いため、
        Excelの行番号から「Z-R行番号」という管理番号を自動生成します。まず「ドライラン実行」で内容を確認してから、
        「取込実行」で実際にデータベースへ登録してください。取込実行は取り消せないため、必ずドライラン結果・
        警告欄を確認してからにしてください。
      </p>
      ...
      <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 0, marginBottom: 12 }}>
        取引先区分は「仕入先」列の文字列から自動判定します(「ブックオフ」「駿河屋」→適格請求書発行事業者(インボイスあり)、
        「手持品」「ラクマ」「メルカリ」「ヤフー」→消費者(個人)、それ以外→適格請求書発行事業者(インボイスあり))。
        既定ステータス・取引先区分は、下記のドライラン結果の表で商品ごとに個別上書きもできます。
      </p>
